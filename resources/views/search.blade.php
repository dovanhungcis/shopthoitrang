		...
		<div
			...
			<div class="page-content__wrapper">
				<div class="heading headin--main">
					<h2 class="hN">Search Results for:</h2>
					<span class="archive__side-title beta">{{$posts->value}}</span>
				</div>
				<hr class="separator" />
								...
								<header class="article__header">
									<div class="article__featured-image" style="padding-top: 66.75%">
										<a href="/post/{{ $post->id }}/{{ $post->slug }}"> <img
											src="/admin1/assets/uploads/posts/{{ $post->img }}"
											alt="{{ $post->title }}" />
									...
									<ol class="nav  article__categories">
										...
										<li><a href="/category/{{ $cate->id }}/{{ $cate->slug }}"
											title="View all posts in {{ $cate->name }}" rel="tag"> {{
												$cate->name }} </a></li> @endforeach
									</ol>
								...
								<section class="article__content entry-summary">
									<a href="/post/{{ $post->id }}/{{ $post->slug }}">
										<p style="text-align: justify;">{{
											Str::words($post->description, 50,'....') }}</p>
									</a>
								</section>
									...
									<span class="meta-box  article__date"> <i class="icon-time"></i>
										<span class="meta-text"><abbr class="published updated"
											title="{{ date('F j, Y',strtotime($post->updated_at)) }}"> {{
												date('F j, Y',strtotime($post->updated_at)) }} </abbr></span>
									</span> <span class="meta-box  article__likes"> <i
										class="icon-heart"></i> <span class="meta-text"> {{ $post->like
											}} </span>
									</span>
								</footer>
							</article>
