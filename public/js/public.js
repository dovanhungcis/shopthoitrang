(function($) {
  ...
  $(function() {
    ...
    $(document).ready(function() {
      if (locals.load_likes_with_ajax == true) {
        $('.pixlikes-box').each(function() {
          var likebox = this, post_id = $(likebox).data('id');
          ...
          jQuery.ajax({
            ...
            data : {
              action : 'pixlikes',
              _ajax_nonce : locals.ajax_nounce,
              type : 'get',
              post_id : post_id
            },
            // beforeSend: function() {jQuery("#loading").show("slow");}, //show
            // loading just when link is clicked
            // complete: function() { jQuery("#loading").hide("fast");}, //stop
            // showing loading when the process is complete
    ...
    if (locals.like_on_action == 'click' || touch) {
      ...
      $(document).on('mouseenter', '.pixlikes-box.likeable .like-link',
          function() {
            var likebox = $(this).parent('.pixlikes-box');
    ...
    var like_this = function(likebox) {
      ...
      if (typeof post_id === 'undefined'
          || (!locals.free_votes && getCookie("pixlikes_" + post_id)))
        return;
      ...
      jQuery.ajax({
        ...
        // beforeSend: function() {jQuery("#loading").show("slow");}, //show
        // loading just when link is clicked
        // complete: function() {}, //stop showing loading when the process is
        // complete
        success : function(response) { // so, if data is retrieved, store it in
                                        // result
          var result = JSON.parse(response);
          if (result.success) {
            $(likebox).find('.likes-count').text(result.likes_number);
